...
-- Idempotent: safe to re-run from scratch (e.g. after a failed seed load
-- left partial state). Drops in reverse dependency order.
...
CREATE TABLE hierarchy_levels (
    ...
    created_at TIMESTAMPTZ DEFAULT NOW()
    -- NOTE: no UNIQUE(org_id, level_number, department) here. A DAG legitimately
    -- has multiple distinct nodes at the same level within the same department
    -- (e.g. level 8 "ortho" has Ortho General, Ortho TKR Unit, and the
    -- multi-parent Post-TKR Protocol Area all as siblings) — `id` is already
    -- the real identity key. See docs/architecture.md.
);
